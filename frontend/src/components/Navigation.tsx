        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-4">
              ...
              {isLoggedIn && (
                <Link to="/wishlist" className={`relative ${darkMode ? 'text-light hover:text-primary' : 'text-gray-700 hover:text-primary'} px-3 py-2 rounded-md text-sm font-medium transition-colors flex items-center gap-1`}>
                  <svg className="w-4 h-4" fill={wishlistCount > 0 ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                  </svg>
                  Wishlist
                  {wishlistCount > 0 && (
                    <span className="ml-1 bg-primary text-white text-xs rounded-full px-1.5 py-0.5 min-w-[1.25rem] text-center">
                      {wishlistCount}
                    </span>
                  )}
                </Link>
