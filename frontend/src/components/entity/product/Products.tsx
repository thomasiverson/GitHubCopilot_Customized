            ...
            {filteredProducts?.map(product => (
              <div key={product.productId} className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg overflow-hidden shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-[0_0_25px_rgba(118,184,82,0.3)] flex flex-col`}>
                ...
                >
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); handleToggleWishlist(product.productId); }}
                    className="absolute top-2 right-2 p-1.5 rounded-full bg-white/80 dark:bg-gray-700/80 hover:bg-white dark:hover:bg-gray-700 shadow transition-colors"
